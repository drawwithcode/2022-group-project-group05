...
//creo l'instance di p5 con il mio sfondo
var bg= function(bg){
  
  bg.setup = function() {

    bgCanvas= bg.createCanvas(300, 300);
    bgCanvas.id("bgCanvas");
    bgCanvas.style("display","block")

    bg.noStroke();
    bg.colorMode(bg.RGB, 255, 255, 255);
    bg.pixelDensity(1)
  ...
  //seed= seed + 0.005;
  //let val1= bg.noise(seed)*800; 
  /*console.log(bgCanvas.height, val1 )

 let gradient= bg.drawingContext.createLinearGradient(bg.width, 0 , bg.width, bg.height);

  gradient.addColorStop(0,bg.color(0));
  gradient.addColorStop(1, bg.color(255));

  bg.drawingContext.fillStyle=gradient;
  bg.rect(0,0,bg.width, bg.height)

  // get the image from bgCanvas
  //const imageData = bg.drawingContext.getImageData( 0, 0, ctx.canvas.width , 100);
...
document.body.appendChild(document.querySelector("#bgCanvas"))
}


var ditherBg= new p5(bg)
